// resources/js/Pages/SampleInvoices/Create.tsx
...
export default function Create({ shippers, customers }: Props) {
  const { data, setData, post, processing, errors } = useForm({
    shipper_id: '',
    customer_id: '',
    date: new Date().toISOString().substr(0, 10),
    buyer_account: '',
    shipment_terms: 'Collect' as 'Collect' | 'Prepaid',
    courier_name: '',
    tracking_number: '',
    items: [{ art_num:'', description:'', size:'', hs_code:'', qty:0, unit_price:0 }] as Item[],
    footnotes: '1. Sample of no commercial value.\n2. Values provided for the Customs Purposes ONLY.',
  });
  ...
  const preview = (e: React.FormEvent) => {
    e.preventDefault();
    post(route('sample-invoices.preview'), { preserveState: true });
  };

  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    post(route('sample-invoices.store'));
  };
  ...
  return (
    <AppLayout breadcrumbs={[
      ...
      <Head title="Create Sample Invoice" />
          ...
          <Table className="min-w-full">
            ...
            <TableBody>
              {data.items.map((itm, i) => {
                const sub = itm.qty * itm.unit_price;
                return (
                  <TableRow key={i}>
                    <TableCell>
                      <Input
