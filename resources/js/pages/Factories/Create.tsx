...
export default function Create() {
  ...
  const removeCertificate = (idx: number) => {
    ...
    setCertNames(updated.map(f => f.name));
  };
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    post(route('factories.store'), { forceFormData: true });
  };
  ...
  return (
    <AppLayout
      breadcrumbs={[
        ...
        { title: 'Create', href: route('factories.create') },
      ]}
    >
      ...
      <div className="min-h-screen bg-gray-50 p-6 md:p-8">
        <div className="max-w-7xl mx-auto">
          <HeadingSmall
            ...
            description="Fill in the details below to create a new factory record."
            className="mb-8"
          />
          ...
          <form onSubmit={submit} encType="multipart/form-data" className="space-y-8">
                ...
                <div className="space-y-2">
                  {certNames.map((name, i) => (
                    <div key={i} className="flex items-center justify-between bg-gray-100 p-2 rounded-md">
                      <span className="text-sm italic text-gray-600">{name}</span>
                      <button type="button" onClick={() => removeCertificate(i)} className="text-red-600 hover:text-red-800">
                        ×
                      </button>
                    </div>
                  ))}
            ...
            <div className="flex items-center justify-between pt-6">
              ...
              {recentlySuccessful && (
                <p className="text-sm text-green-600">Factory created successfully!</p>
              )}
            </div>
          </form>
        </div>
