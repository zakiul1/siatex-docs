// resources/js/Pages/SalesInvoices/Edit.tsx
...
export default function Edit() {
  const { shippers, customers, banks, invoice: inv } = usePage<{
    shippers: Shipper[]
    customers: Customer[]
    banks: Bank[]
    invoice: any
  ...
  const { data, setData, put, processing, errors } = useForm({
    shipper_id:       inv.shipper_id?.toString()      || '',
    customer_id:      inv.customer_id?.toString()     || '',
    invoice_no:       inv.invoice_no?.toString()      || '',
    type:             inv.type                        || 'LC',
    issue_date:       inv.issue_date.substr(0,10),
    delivery_date:    inv.delivery_date
                          ? inv.delivery_date.substr(0,10)
                          : '',
    payment_mode:     inv.payment_mode                || '',
    currency:         inv.currency                    || 'USD',
    siatex_discount:  inv.siatex_discount?.toString() || '0',
    items:            inv.items.length
      ? inv.items.map((i: any) => ({
          ...
          commercial_cost: i.commercial_cost?.toString() || '0',
        }))
      : [blankRow],
  })

  // Sync payment_mode when type toggles
  useEffect(() => {
    setData(
      'payment_mode',
      data.type === 'LC'
  ...
  function handleUpdate(e: React.FormEvent) {
    e.preventDefault()
    put(route('sales-invoices.update', inv.id))
  }

  // CSRF for the plain preview form
  const csrf = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || ''

  return (
    <AppLayout>
      <Head title={`Edit Invoice #${inv.invoice_no}`} />

      <form onSubmit={handleUpdate} className="p-6 bg-white space-y-6">
        {/* Breadcrumb & Title */}
        <div>
          <nav className="text-sm text-gray-600 mb-1">
            <ol className="flex space-x-2">
              ...
              <li>/</li>
              <li>
                <Link href={route('sales-invoices.index')} className="hover:underline">
                  Sales Invoices
                </Link>
              </li>
              <li>/</li>
          ...
          <h1 className="text-2xl font-semibold">Edit Invoice #{inv.invoice_no}</h1>
        </div>

        {/* Select Shipper / Customer / Type */}
          ...
          <div className="w-1/3">
            ...
            >
              <div className="flex items-center space-x-1">
                <RadioGroupItem id="edit-lc" value="LC" />
                <label htmlFor="edit-lc">L/C Invoice</label>
              </div>
              <div className="flex items-center space-x-1">
                <RadioGroupItem id="edit-tt" value="TT" />
                <label htmlFor="edit-tt">TT Invoice</label>
              </div>
            </RadioGroup>
        ...
        {/* Header: Shipper Details & Invoice No. */}
              ...
              <tr key={i}>
                ...
                <td className="border p-1">
                  <Input
                    ...
                    onChange={e => {
                      const arr = [...data.items]
                      arr[i].unit_price = e.target.value || '0'
                      setData('items', arr)
                    }}
                  />
                ...
                <td className="border p-1">
                  <Input
                    ...
                    onChange={e => {
                      const arr = [...data.items]
                      arr[i].commercial_cost = e.target.value || '0'
                      setData('items', arr)
                    }}
                  />
